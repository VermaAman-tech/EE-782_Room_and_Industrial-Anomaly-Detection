...
flowchart LR
  ...
  subgraph PRE[Preprocessing Layer]
    IMG_PRE[Image Preprocessing<br/>denoise, wavelets, flow]
    AUD_PRE[Audio Preprocessing<br/>resample, STFT, mel, CWT]
  end
  ...
  subgraph VIS[Visual Analysis]
    ...
    SEG[SAM Segmenter]
    FLOW[Optical Flow & Motion Analysis]
  end
  ...
  subgraph FUS[Fusion Layer]
    PROJ[Projection & Positional Embeddings]
    XATT[Cross-Modal Transformer<br/>Visual↔Audio Attention]
    POOL[Temporal Pooling]
    HEADS[Task Heads<br/>Motion / Event Classifier]
  end

  %% Anomaly & Outputs
  subgraph OUT[Output Layer]
    ANOM[Anomaly Scoring<br/>(statistical / AE / audio)]
    LOG[Event Logging & Storage]
    DASH[Streamlit Dashboard<br/>Live + Post-Processing]
    ARTIFACTS[Saved Artifacts<br/>frames/*.jpg & audio/*.wav]
  end

  %% Dataflow connections
  CAM -->|frames w/ timestamps| IMG_PRE
  MIC -->|audio stream| AUD_PRE
  ...
  %% Optional arrows for post-processing / feedback
  LOG -->|feedback / retrain| models_update[(Model Update Pipeline)]
  models_update -.-> DET
  models_update -.-> AST
  models_update -.-> XATT

  classDef layer fill:#f8f9fa,stroke:#333,stroke-width:1px;
  class SENSORS,PRE,VIS,AUD,FUS,OUT layer;
